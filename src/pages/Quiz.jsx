      ...
      <div className="flex-1 bg-[#f3eaf7] px-4 py-12">
        <div className="max-w-6xl mx-auto">
          ...
          <motion.div className="text-center mb-10"
            ...
            <h2 className="text-2xl md:text-3xl font-semibold text-purple-700">வேதாகம தேர்வு</h2>
          ...
          {!loading && !error && items.length === 0 && (
            <motion.div className="text-center text-gray-400 py-20"
              ...
              <p className="text-sm">விரைவில் தேர்வு கேள்விகள் வரும்.</p>
            </motion.div>
          )}
              ...
              {items.map((item, i) => (
                <motion.div key={item.id}
                  ...
                  <div className="p-5 text-center">
                    <p className="text-sm font-medium text-gray-700 mb-5">{item.title}</p>
                    <div className="flex flex-col sm:flex-row justify-center gap-3">
                      {item.questions_file_url && (
                        <motion.a href={item.questions_file_url} target="_blank" rel="noopener noreferrer"
                          className="flex items-center justify-center gap-2 px-4 py-3 text-sm rounded-md text-white bg-gradient-to-r from-purple-500 to-purple-700 min-h-[44px]"
                          whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.93 }}>
                          <FaQuestionCircle /> கேள்விகள்
                        </motion.a>
                      )}
                      {item.answers_file_url && (
                        <motion.a href={item.answers_file_url} target="_blank" rel="noopener noreferrer"
                          className="flex items-center justify-center gap-2 px-4 py-3 text-sm rounded-md text-white bg-gradient-to-r from-purple-500 to-purple-700 min-h-[44px]"
                          whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.93 }}>
                          <FaCheckCircle /> விடைகள்
                        </motion.a>
                      )}
                    </div>
                  </div>
